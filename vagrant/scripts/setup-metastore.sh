...
function setupHiveSite {  
    ...
    /vagrant/resources/hive/hive-props.sh
}
...
function startMetastore {
    ...
    sudo -u vagrant nohup $HADOOP_HOME/bin/hive --service metastore < /dev/null > /usr/local/hive/logs/hive_metastore_`date +"%Y%m%d%H%M%S"`.log 2>&1 &
    #su -u vagrant "$HIVE_HOME/bin/hive --service hiveserver2 &"
    sudo -u vagrant nohup $HADOOP_HOME/bin/hive --service hiveserver2 < /dev/null > /usr/local/hive/logs/hive_server2_`date +"%Y%m%d%H%M%S"`.log 2>&1 &
}
